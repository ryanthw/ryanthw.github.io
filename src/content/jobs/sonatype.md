---
company: Sonatype
role: Associate Software Engineering Intern
location: Remote
start: May 2026
end: Aug 2026
order: 100
defaultOpen: true
summary: Migrated Lifecycle infrastructure from EKS to ECS Fargate and managed the AWS estate with Terraform.
groups:
  - heading: Cloud infrastructure
    bullets:
      - Played a key role in migrating the Lifecycle product from EKS/Cloudy to ECS Fargate, improving the efficiency and scalability of its cloud footprint.
      - Managed AWS resources extensively with Terraform — deploying, managing and destroying personal instances of the product on ECS to support robust cloud testing inside the SDLC.
  - heading: Observability
    bullets:
      - Assisted the transition from Datadog to the Observe platform, migrating and configuring the essential monitors and charts.
  - heading: Team
    bullets:
      - Contributed within the SCA team in a sprint-based environment, delivering directly into the Lifecycle product.
stack:
  - AWS
  - ECS Fargate
  - EKS
  - Terraform
  - Observe
  - Datadog
  - Java
note: Written at a level that stays within what is publicly shareable — no internal architecture, screenshots or source detail.
evidence: []
---
